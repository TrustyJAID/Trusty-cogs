...
async def make_rules_embed(guild, team, rules):
    ...
    """
    warning = _("***Violating [Discord Terms of Service](https://discordapp.com/terms) "
...
async def make_leaderboard_embed(guild, post_list, page=0):
    ...
    em.set_author(name=guild.name + _(" Pickems ") + style + _(" Leaderboard"), icon_url=guild.icon_url)
    em.set_thumbnail(url=guild.icon_url)
...
async def roster_embed(post_list, page):
    ...
    if position != "Goalie":
        post_data = {_("Shots") : year_stats["stat"]["shots"],
